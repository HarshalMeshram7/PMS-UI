...
export const TeamDetailsDialog = ({
  ...
  useLayoutEffect(() => {
    let { teamplayersTypes, teamstaffsTypes, teamcoachesTypes } = teamMembers;

    let tempPlayer =[], tempPlayerID=[], tempStaff=[], tempStaffID=[], tempCoach=[], tempCoachID=[];

    teamstaffsTypes?.map((item) => {
      tempStaff?.push({
        name: item.StaffName,
        types: item.StaffTypeDec,
      });
      tempStaffID?.push({
        staffID: item.StaffID,
        staffTypeIDs: item.StaffTypeID,
      });
    });
     teamcoachesTypes?.map((item) => {
      tempCoach?.push({
        name: item.CoachName,
        types: item.CoachTypeDec,
      });
      tempCoachID?.push({
        coachID: item.CoachID,
        coachTypeIDs: item.CoachTypeID,
      });
    });
     teamplayersTypes?.map((item) => {
      tempPlayer?.push({
        name: item.PlayerName,
        type: item.PlayerTypeDec,
      });
      tempPlayerID?.push({
        playerID: item.PlayerID,
        playerTypeID: item.PlayerTypeID,
      });
    });
    setStaffArrayTable(tempStaff);
    setCoachArrayTable(tempCoach);
    setPlayersArrayTable(tempPlayer);
  ...
  const savePlayers = () => {
    let playerID = formik.values.teamplayerID.ID,
  ...
  const saveStaffs = async () => {
    let staffID = formik.values.teamstaffID.ID,
      staffName = getFullName(
        formik.values.teamstaffID.FirstName,
        formik.values.teamstaffID.LastName
      ),
  ...
  const saveCoachs = () => {
    let coachID = formik.values.teamcoachID.ID,
      coachName = getFullName(
        formik.values.teamcoachID.FirstName,
        formik.values.teamcoachID.LastName
      ),
  ...
  const handleRemove = (item, index, from) => {
    ...
    //   for UI
    let tableArray = selectName[`${from}`].tableArray,
                    ...
                    <Grid container spacing={8}>
                      <Grid item lg={6} md={6} xs={6}>
                        <CardActions>
                          ...
                          <Button
                            onClick={() => {
                              document.getElementById("uploadTeamLogo").click();
                            }}
                          >
                            Upload Logo
                          </Button>
                      ...
                      <Grid item lg={6} md={6} xs={6}>
                        <CardActions>
                          ...
                          <Button
                            onClick={() => {
                              document.getElementById("uploadTeamBanner").click();
                            }}
                          >
                            Upload Banner
                          </Button>
                    ...
                    <CardContent>
                      <Grid container spacing={3}>
                        ...
                        <Grid item md={6} xs={12}>
                          <TextField
                            error={Boolean(
                              formik.touched.managerPhoneNo && formik.errors.managerPhoneNo
                            )}
                            fullWidth
                            helperText={
                              formik.touched.managerPhoneNo && formik.errors.managerPhoneNo
                ...
                <Grid item lg={12} md={12} xs={12}>
                  <Card>
                    <CardContent>
                      <Grid container spacing={8}>
                        {/* select team players */}
                        {playersArrayTable && playersArrayTable?.length !== 0 && (
                          <TeamTable
                        ...
                        <Grid item md={4} xs={12}>
                          <Button onClick={savePlayers} variant="contained">
                            Add Player
                          </Button>
                        </Grid>

                        {staffArrayTable?.length !== 0 && (
                          <TeamTable
                        ...
                        <Grid item md={4} xs={12}>
                          <Button onClick={saveStaffs} variant="contained">
                            Add Staff
                          </Button>
                        </Grid>
                        {coachArrayTable?.length !== 0 && (
                          <TeamTable
                        ...
                        <Grid item md={4} xs={12}>
                          <Button textAlign="right" onClick={saveCoachs} variant="contained">
                            Add Coach
                          </Button>
                        </Grid>
                      </Grid>
